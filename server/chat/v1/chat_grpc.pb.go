...
const (
	...
	ChatService_History_FullMethodName    = "/chat.v1.ChatService/History"
...
type ChatServiceClient interface {
	...
	ChatStream(ctx context.Context, in *HistoryRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[StreamEnvelope], error)
}
...
func (c *chatServiceClient) ChatStream(ctx context.Context, in *HistoryRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[StreamEnvelope], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &ChatService_ServiceDesc.Streams[0], ChatService_ChatStream_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[HistoryRequest, StreamEnvelope]{ClientStream: stream}
...
type ChatService_ChatStreamClient = grpc.ServerStreamingClient[StreamEnvelope]
...
type ChatServiceServer interface {
	...
	ChatStream(*HistoryRequest, grpc.ServerStreamingServer[StreamEnvelope]) error
...
func (UnimplementedChatServiceServer) ChatStream(*HistoryRequest, grpc.ServerStreamingServer[StreamEnvelope]) error {
...
func _ChatService_ChatStream_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(ChatServiceServer).ChatStream(m, &grpc.GenericServerStream[HistoryRequest, StreamEnvelope]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type ChatService_ChatStreamServer = grpc.ServerStreamingServer[StreamEnvelope]
